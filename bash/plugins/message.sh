...
function abcli_message() {
    ...
    if [ "$task" == "help" ] ; then
        abcli_help_line "abcli message listen_to <host_name>" \
            "listen to host_name messages."
        abcli_help_line "abcli message listen_as <host_name>" \
            "listen to messages as host_name."
        abcli_help_line "abcli message submit [--data <data>] [--filename <filename>] [--recipient <host_1,host_2>] [--subject <subject>]" \
