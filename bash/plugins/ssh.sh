...
function abcli_ssh() {
    ...
    if [ "$task" == "help" ] ; then
        abcli_help_line "ssh ec2/jetson_nano/rpi 1-2-3-4 [./vnc/worker/worker,gpu] [region=region_1,user=ec2-user/ubuntu]" \
    ...
    if [ -z "$address" ] ; then
        abcli_log_error "ssh address unknown."
        return
    fi
    ...
    if [ "$kind" == "ec2" ] ; then
       ...
       abcli_log_error "unknown kind: ssh '$kind'."
    fi
}