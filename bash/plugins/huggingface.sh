...
function abcli_huggingface() {
    ...
    if [ "$task" == "help" ] ; then
        abcli_help_line "$abcli_cli_name huggingface clone <repo_name>" \
            "clone huggingface/repo_name."
        abcli_help_line "$abcli_cli_name huggingface install" \
            "install huggingface."
        abcli_help_line "$abcli_cli_name huggingface get_model_path <repo_name> [<model_name>] [model=object/*saved]" \
            "return model_path for saved/object model repo_name/model_name."
        abcli_help_line "$abcli_cli_name huggingface save <repo_name> <model_name> <object_name> [force]" \
