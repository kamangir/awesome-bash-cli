...
function abcli_tag() {
    ...
    if [ "$task" == "help" ] ; then
        abcli_help_line "$abcli_name tag clone object_1 object_2" \
            "clone object_1 tags -> object_2."
        abcli_help_line "$abcli_name tag get object_1" \
            "get object_1 tags."
        abcli_help_line "$abcli_name tag search tag_1" \
            "search for all objects that are tagged tag_1."
        abcli_help_line "$abcli_name tag set object_1,object_2 tag_1,~tag_2 [validate]" \
