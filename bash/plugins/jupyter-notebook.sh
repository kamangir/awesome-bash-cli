...
function abcli_notebook() {
    ...
    if [ "$task" == "help" ] ; then
        abcli_help_line "notebook [browse] [notebook] [args]" \
            "browse $abcli_asset_name/notebook.ipynb [and pass args]."
        abcli_help_line "notebook build [notebook]" \
            "build $abcli_asset_name/notebook.ipynb."
        abcli_help_line "notebook connect 1-2-3-4 [setup]" \
            "[setup and] connect to jupyter notebook on ec2:1-2-3-4."
        abcli_help_line "notebook host [setup]" \
    ...
    abcli_log_error "unknown task: notebook '$task'."
}
