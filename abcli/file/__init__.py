...
def load(filename, civilized=True, default={}):
    ...
    Args:
        filename (str): filename.
        civilized (bool, optional): if failed, do not print error message. Defaults to True.
...
def load_image(filename, civilized=True):
...
def load_json(filename, civilized=True, default={}):
...
def load_text(filename, civilized=True, count=-1):
